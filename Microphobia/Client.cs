...
namespace N17Solutions.Microphobia
{
    public class Client : IHostedService, IDisposable
    {
        ...
        public Client(Queue queue, MicrophobiaConfiguration config, ILogger<Client> logger, ISystemLogProvider systemLogProvider)
        {
            _queue = queue;
            _config = config;
            _logger = logger;
                    ...
                    var task = Task.Run(async () =>
                    {
                        while (true)
                        {
                            ...
                            if (nextTask != null)
                            {
                                ...
                                {
                                    await LogTaskException(nextTask, ex).ConfigureAwait(false);
                                }
                            }
                            else
                            {
